...
public static class WinMoeButtonVisualState
{
    ...
    {
        if (root is Button button && IsWinMoeButton(button))
        {
            Freeze(button);
        }
    ...
    private static bool IsWinMoeButton(Button button)
    {
        return IsWinMoeStyle(button.Style);
    }

    private static bool IsWinMoeStyle(Style? style)
    {
        while (style is not null)
        {
            foreach (var key in WinMoeButtonStyleKeys)
            {
