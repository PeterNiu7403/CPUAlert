...
{
    private readonly string _tempRoot = Path.Combine(Path.GetTempPath(), "MoleWindowsTests", Guid.NewGuid().ToString("N"));
    ...
    public void Constructor_UsesNormalizedDefaults_WhenFileIsMissing()
    {
        ...
        Assert.Equal(MoleWindowsSettings.DefaultSamplingIntervalSeconds, service.Current.SamplingIntervalSeconds);
        Assert.Equal(MoleWindowsSettings.DefaultHistoryRetentionDays, service.Current.HistoryRetentionDays);
        Assert.Equal(MoleWindowsSettings.DefaultHttpServerPort, service.Current.HttpServerPort);
    ...
    {
        var service = new JsonApplicationSettingsService(_settingsPath);
        MoleWindowsSettings? changedSettings = null;
        service.SettingsChanged += (_, settings) => changedSettings = settings;

        var saved = await service.SaveAsync(new MoleWindowsSettings
        {
