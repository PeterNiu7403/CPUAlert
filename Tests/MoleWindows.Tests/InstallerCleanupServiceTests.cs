...
public sealed class InstallerCleanupServiceTests : IDisposable
{
    ...
    {
        _root = Path.Combine(Path.GetTempPath(), "MoleWindowsInstallerTests", Guid.NewGuid().ToString("N"));
    ...
    {
        var outside = Path.Combine(Path.GetTempPath(), $"molewindows-outside-{Guid.NewGuid():N}.msi");
